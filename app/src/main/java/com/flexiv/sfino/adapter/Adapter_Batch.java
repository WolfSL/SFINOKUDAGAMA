...
public class Adapter_Batch extends RecyclerView.Adapter<Adapter_Batch.ViewsHolder> implements Filterable {
    ...
    public void onBindViewHolder(@NonNull ViewsHolder holder, int position) {
        ...
        holder.card_name.setText(SharedPreference.df.format(item.getPrice()));
        holder.card_code.setText(item.getCode());
        holder.card_code2.setText(SharedPreference.df.format(item.getSih()));
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (Modal_Batch c : arr_full
                    ...
                    if(c.getCode().toLowerCase().contains(filterPattern)){
                        filteredList.add(c);
                    }
                }
