...
public class Adapter_INV_list extends RecyclerView.Adapter<Adapter_INV_list.ViewsHolder> implements Filterable {

    private ArrayList<TBLT_SALINVHED> arr;
    private ArrayList<TBLT_SALINVHED> arr_full;
    private int type;
    private Context con;
    ...
    public Adapter_INV_list(ArrayList<TBLT_SALINVHED> arr, int type, Context con) {
        this.arr = arr;
    ...
    public void onBindViewHolder(@NonNull ViewsHolder holder, int position) {
        TBLT_SALINVHED orderhed = arr.get(position);
    ...
    private Filter filter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ArrayList<TBLT_SALINVHED> filteredList = new ArrayList<>();

            if (constraint == null || constraint.length() == 0) {
                filteredList.addAll(arr_full);
            } else {
                String filterPattern = constraint.toString().toLowerCase().trim();
                for (TBLT_SALINVHED c : arr_full
                ) {
